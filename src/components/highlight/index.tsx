...
function escape(string: string) {
  return string && reHasUnescapedHtml.test(string)
    ? string.replace(reUnescapedHtml, (chr: string) => htmlEscapes[chr as keyof typeof htmlEscapes])
    : string || '';
}
...
export function Highlight(props: PropsWithChildren<HighlightProps>) {
  ...
  const codeBlock = <div dangerouslySetInnerHTML={{ __html: escape(children + '') }} />;
  useEffect(() => {
    if (nodeRef.current) {
